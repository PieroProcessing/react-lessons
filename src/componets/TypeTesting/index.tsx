...
const withInit = (dispatchFN: Function, Component: React.FC) =>
  ({ ...props }): JSX.Element => {
    useEffect(() => {
      dispatchFN();
    }, []);
...
const TypeTesting = (): JSX.Element => {
  ...
  const emptyCart: EmptyCart = { unpaidItems: [] };
  const listCart: ActiveCartData = { unpaidItems: items };
  const paidList: PaidCartData = { paidItems: items, payment: 9 };
